...
var _ carbonpanelv1connect.ProxyServiceHandler = (*ProxyService)(nil)
...
// GetVelocitySecret retrieves or lazily auto-generates the modern forwarding secret
func (s *ProxyService) GetVelocitySecret(ctx context.Context, req *connect.Request[v1.GetVelocitySecretRequest]) (*connect.Response[v1.GetVelocitySecretResponse], error) {
	secret, err := s.store.GetSystemSetting(ctx, minecraft.SettingVelocitySecret)
	autoGen := false
	if err != nil || secret == "" {
		newSec, genErr := minecraft.GenerateVelocitySecret()
		if genErr != nil {
			return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("failed to generate secret: %w", genErr))
		}
		if setErr := s.store.SetSystemSetting(ctx, minecraft.SettingVelocitySecret, newSec); setErr != nil {
			return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("failed to persist secret: %w", setErr))
		}
		secret = newSec
		autoGen = true
	}

	return connect.NewResponse(&v1.GetVelocitySecretResponse{
		Secret:        secret,
		AutoGenerated: autoGen,
	}), nil
}

// RotateVelocitySecret generates a new secret (or applies a custom one) and syncs to backend Paper servers
func (s *ProxyService) RotateVelocitySecret(ctx context.Context, req *connect.Request[v1.RotateVelocitySecretRequest]) (*connect.Response[v1.RotateVelocitySecretResponse], error) {
	newSecret := req.Msg.GetCustomSecret()
	if newSecret == "" {
		generated, err := minecraft.GenerateVelocitySecret()
		if err != nil {
	...
	if err := s.store.SetSystemSetting(ctx, minecraft.SettingVelocitySecret, newSecret); err != nil {
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("failed to save new secret: %w", err))
	}

	syncedServers := make([]string, 0)
	if req.Msg.GetSyncAllServers() {
		servers, err := s.store.ListServers(ctx)
		if err == nil {
			for _, srv := range servers {
				if srv.DataPath == "" {
				...
				if _, syncErr := minecraft.SyncVelocitySecretToDataPath(srv.DataPath, newSecret); syncErr == nil {
					syncedServers = append(syncedServers, srv.ID)
				}
			}
		}
	}

	return connect.NewResponse(&v1.RotateVelocitySecretResponse{
		NewSecret:          newSecret,
		SyncedServersCount: int32(len(syncedServers)),
		SyncedServerIds:    syncedServers,
		Message:            fmt.Sprintf("Velocity secret rotated successfully and synced to %d servers", len(syncedServers)),
	}), nil
}

// SyncVelocitySecretToServer updates a specific server's config files with the active forwarding secret
func (s *ProxyService) SyncVelocitySecretToServer(ctx context.Context, req *connect.Request[v1.SyncVelocitySecretToServerRequest]) (*connect.Response[v1.SyncVelocitySecretToServerResponse], error) {
	secret, err := s.store.GetSystemSetting(ctx, minecraft.SettingVelocitySecret)
	if err != nil || secret == "" {
		newSec, genErr := minecraft.GenerateVelocitySecret()
		if genErr != nil {
		...
		_ = s.store.SetSystemSetting(ctx, minecraft.SettingVelocitySecret, newSec)
		secret = newSec
	}

	server, err := s.store.GetServer(ctx, req.Msg.GetServerId())
	if err != nil {
		return nil, connect.NewError(connect.CodeNotFound, fmt.Errorf("server not found: %s", req.Msg.GetServerId()))
	}

	updatedFile, err := minecraft.SyncVelocitySecretToDataPath(server.DataPath, secret)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("failed to sync secret to server: %w", err))
	}

	return connect.NewResponse(&v1.SyncVelocitySecretToServerResponse{
		ServerId:    server.ID,
		Success:     true,
