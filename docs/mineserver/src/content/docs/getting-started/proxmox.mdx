---
title: Proxmox LXC
description: Deploy MINESERVER in a Proxmox LXC container using the community helper script.
---
...
The [Proxmox VE Community Scripts](https://community-scripts.github.io/ProxmoxVE/) project maintains a [MINESERVER helper script](https://community-scripts.github.io/ProxmoxVE/scripts?id=MINESERVER) that creates a Debian 13 LXC container with everything pre-installed (Docker, Go, Node.js, MINESERVER built from source) and managed as a systemd service.

From your **Proxmox host shell**:

```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/MINESERVER.sh)"
```
...
Since MINESERVER runs directly on the host here (not inside a Docker container), file paths are straightforward — `MINESERVER_DATA_DIR` and `MINESERVER_HOST_DATA_PATH` should be the same value. The default `./data` relative to `/opt/MINESERVER` works out of the box.

Once complete, MINESERVER is accessible at **http://\<container-ip\>:8080**.
