...
// actionTable maps every known procedure (referenced through the generated
// connect constants so renames break the build) to its deterministic action
// name "<resource>.<verb>".
var actionTable = map[string]string{
	cloudv1connect.AgentServiceConnectProcedure:                 "agent.connect",
	cloudv1connect.AgentServiceJoinNodeProcedure:                "nodes.join",
	cloudv1connect.AgentServiceRenewCredentialsProcedure:        "nodes.renew_credentials",
...
func defaultResourceType(procedure string) string {
	...
	for _, prefix := range []struct {
		...
		{"workloads.", "workloads"}, {"agent.", "nodes"},
	} {
