<script lang="ts">
	...
	import { UpdateAuthSettingsRequestSchema } from '$lib/proto/mineserver/v1/auth_pb';
	import type { GetAuthConfigResponse } from '$lib/proto/mineserver/v1/auth_pb';
	import {
		Shield,
		Loader2,
		Check,
...
{:else if config}
  ...
  mineserver:
    environment:
      mineserver_AUTH_OIDC_ENABLED: "true"
      mineserver_AUTH_OIDC_ISSUER_URI: "https://..."
      mineserver_AUTH_OIDC_CLIENT_ID: "mineserver"
      mineserver_AUTH_OIDC_CLIENT_SECRET: "your-secret"
      mineserver_AUTH_OIDC_REDIRECT_URL: "https://..."
      mineserver_AUTH_OIDC_SCOPES: "openid,profile,email,groups"
      mineserver_AUTH_OIDC_ROLE_CLAIM: "groups"</code
									></pre>
							</AccordionContent>
						</AccordionItem>
						...
						<AccordionItem value="env-vars">
							...
							<AccordionContent>
								<div class="space-y-1.5 font-mono text-xs">
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5">mineserver_AUTH_OIDC_ENABLED</code>
									</p>
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5"
											>mineserver_AUTH_OIDC_ISSUER_URI</code
										>
									</p>
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5"
											>mineserver_AUTH_OIDC_CLIENT_ID</code
										>
									</p>
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5"
											>mineserver_AUTH_OIDC_CLIENT_SECRET</code
										>
									</p>
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5"
											>mineserver_AUTH_OIDC_REDIRECT_URL</code
										>
									</p>
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5">mineserver_AUTH_OIDC_SCOPES</code>
									</p>
									<p>
										<code class="rounded bg-muted px-1.5 py-0.5"
											>mineserver_AUTH_OIDC_ROLE_CLAIM</code
										>
									</p>
								</div>
							</AccordionContent>
					...
					<p class="text-xs text-muted-foreground">
						Provider examples are available in the <a
							href="https://docs.mineserver.app/introduction/"
							target="_blank"
							rel="noopener noreferrer"
							class="underline hover:text-foreground">mineserver docs</a
						>.
					</p>
				{/if}
			</CardContent>
